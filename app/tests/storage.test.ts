...
describe('custom cards', () => {
  ...
  it('refuses to overwrite an existing custom card with a different name (I2 slug collision)', () => {
    saveCustomCard(card());
    expect(() => saveCustomCard(card({ name: 'Renamed' }))).toThrow(/already used by "Test"/);
    expect(loadCustomCards()).toEqual([card()]);
  });
  it('refuses to save a card whose id collides with a curated card (I2 — it would be invisible)', () => {
    ...
    expect(() => saveCustomCard(card({ id: 'ember-bolt', name: 'Ember Bolt' }))).toThrow(/curated card/);
  ...
  it('saves a card once renamed to a free slug (I2)', () => {
    ...
    expect(() => saveCustomCard(card({ id: 'alpha-strike', name: 'Alpha Strike!' }))).toThrow();
    saveCustomCard(card({ id: 'beta-strike', name: 'Beta Strike' }));
    expect(loadCustomCards().map((c) => c.id)).toEqual(['alpha-strike', 'beta-strike']);
  });
  it('deletes by id', () => {
    saveCustomCard(card());
  ...
  it('blocks deleting a card referenced by another custom card effect (C2)', () => {
    ...
    saveCustomCard(card({ // A references B via copyCard
      id: 'custom-002',
      name: 'Other',
      triggers: [{ when: 'battlecry', effects: [{ kind: 'copyCard', cardId: 'custom-001' }] }],
    }));
    expect(() => deleteCustomCard('custom-001')).toThrow(/Cannot delete custom-001: referenced by custom-002/);
...
describe('shape-checked reads (M5)', () => {
  it('falls back to the empty default for valid-JSON-wrong-shape storage', () => {
    ...
    expect(loadDecks()).toEqual({});
    localStorage.setItem('tcg.customCards', JSON.stringify({ not: 'an array' }));
    expect(loadCustomCards()).toEqual([]);
...
describe('JSON import/export', () => {
  ...
  it('import rejects a card whose summon effect references an unknown card (C1 pool rule)', () => {
    const bad = card({
      id: 'custom-summon',
      triggers: [{ when: 'battlecry', effects: [{ kind: 'summon', value: 1, cardId: 'token-nonexistent' }] }],
    });
  ...
  it('import rejects a card whose copyCard effect references an unknown card (C1)', () => {
    const bad = card({
      id: 'custom-copy',
      triggers: [{ when: 'battlecry', effects: [{ kind: 'copyCard', cardId: 'nope' }] }],
    });
    expect(() => importCardsJson(JSON.stringify([bad]))).toThrow(
  ...
  it('import accepts a summon to a known curated card (C1)', () => {
    const ok = card({
      id: 'custom-summon-ok',
      triggers: [{ when: 'battlecry', effects: [{ kind: 'summon', value: 1, cardId: 'token-rat' }] }],
    });
  ...
  it('import rejects a null element with the documented error, not a TypeError (I5)', () => {
    expect(() => importCardsJson('[null]')).toThrow('Invalid card ?: malformed card data');
  });
  it('import rejects token-archetype cards (M6)', () => {
    const token = card({ id: 'token-rat', name: 'Giant Rat', archetype: 'token' });
