...
export const RARITY_COPY_LIMIT: Record<Rarity, number> = {
	common: 3,
	rare: 2,
	epic: 1,
	legendary: 1,
...
export function statBudget(cost: number): number {
	return 2 + 2 * cost;
}
...
const CREATURE_ONLY_KEYWORDS: Keyword[] = [
	"taunt",
	"rush",
	"charge",
	"windfury",
...
export function validateCard(card: Card): ValidationIssue[] {
	...
	if (!/^[a-z0-9-]{3,32}$/.test(card.id))
		err("id", "ID must be lowercase letters, digits, dashes (3-32 chars).");
	if (!Number.isInteger(card.cost) || card.cost < 0 || card.cost > 15)
		err("cost", "Cost must be an integer 0..15.");
	if (card.type === "creature") {
		if (
			...
			err("attack", "Creature needs attack >= 0.");
		if (
			card.health === undefined ||
			!Number.isInteger(card.health) ||
			card.health < 1
		)
			err("health", "Creature needs health >= 1.");
		if (
			card.reflect === undefined ||
			!Number.isInteger(card.reflect) ||
			card.reflect < 0
		)
			err("reflect", "Creature needs reflect >= 0.");
	} else if (
		card.attack !== undefined ||
		card.health !== undefined ||
		card.reflect !== undefined
	...
	for (const tg of card.triggers ?? []) {
		if (tg.when === "battlecry" && card.type !== "creature")
			err("trigger", "Battlecry is creature-only.");
		if (tg.when === "onDamage" && card.type !== "creature")
			err("trigger", "OnDamage is creature-only.");
		if (tg.effects.length === 0)
			err("effect", `Trigger ${tg.when} requires at least one effect.`);
	}
	if (card.type === "spell" && (card.triggers ?? []).length > 0)
		err("trigger", "Spells cannot have triggers.");
	if (card.type === "spell" && (card.effects ?? []).length === 0)
		err("effect", "Spells require at least one effect.");
	if (card.type !== "spell" && (card.effects ?? []).length > 0)
		err(
			"effect",
			"Only spells use top-level effects; creatures/artifacts use triggers.",
		);
	for (const k of card.keywords) {
		if (k === "ward" && card.keywords.includes("taunt"))
			err("keywords", "Ward and Taunt cannot combine.");
		if (card.type !== "creature" && CREATURE_ONLY_KEYWORDS.includes(k))
			err("keywords", `${k} is creature-only.`);
	}
	if (
		card.type === "creature" &&
		card.attack !== undefined &&
		...
		if (spent > ceiling) {
			err(
				"stats",
				`Attack/Reflect/Health weighted total ${spent} exceeds the max ${ceiling} for cost ${card.cost} ` +
					`(vanilla budget ${budget} + ${STAT_BUDGET_SLACK} design slack) by ${spent - ceiling}.`,
			);
		}
	}
	const TARGET_KINDS: EffectKind[] = [
		"dealDamage",
		"heal",
		"buff",
		"freeze",
	...
	for (const e of allEffects) {
		if (TARGET_KINDS.includes(e.kind) && !e.target)
			err("effect", `${e.kind} requires a target.`);
	}
	return issues;
}
...
export function validateDeck(
	...
	for (const id of cardIds) {
		...
		if (!card) {
			issues.push({
				field: "deck",
				message: `Unknown card id: ${id}`,
				severity: "error",
			});
			continue;
		}
		if (card.archetype === "token")
			issues.push({
				field: "deck",
				message: `Token cards cannot be in decks: ${id}`,
				severity: "error",
			});
		const n = (counts.get(id) ?? 0) + 1;
		counts.set(id, n);
		if (n > RARITY_COPY_LIMIT[card.rarity])
			issues.push({
				field: "deck",
				message: `More than ${RARITY_COPY_LIMIT[card.rarity]} copies of ${card.name} (${card.rarity}).`,
				severity: "error",
			});
	}
	if (cardIds.length !== 60)
		issues.push({
			field: "deck",
			message: `Deck must be exactly 60 cards (has ${cardIds.length}).`,
			severity: "error",
		});
	return issues;
}
