...
export interface Room {
  ...
  deckIds: string[];       // host's 60-card deck — used for BOTH players in v1
  customCards: Card[];
  heroId: string;
  seed: number;
...
export class RoomRegistry {
  ...
  join(code: string, socket: WebSocket): void {
  ...
  private makeGame(room: Room): Game {
    ...
    // sends the hero name it picked from the Forge picker). Both players use
    // the host's hero + deck (v1 mirror match).
