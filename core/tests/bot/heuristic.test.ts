...
describe("evaluate", () => {
	...
	it("values higher own HP and lower enemy HP", () => {
		const a = Game.create(makeTestSetup());
		const b = a.clone();
		b.state.players[0].hero.hp = 10;
		b.state.players[1].hero.hp = 30;
		expect(evaluate(a, 0)).toBeGreaterThan(evaluate(b, 0));
	});
	...
	it("Veteran/Grandmaster play the higher-Reflect creature when other dimensions are equal", () => {
		...
		expect(Veteran.chooseIntent(game, 0)).toMatchObject({
			kind: "playCard",
			handIndex: 1,
		});
		expect(Grandmaster.chooseIntent(game, 0)).toMatchObject({
			kind: "playCard",
			handIndex: 1,
		});
	});
	it("hero attacks ignore Reflect — the higher-Attack creature takes lethal", () => {
		...
		const strong = addCreature(game, 0, {
			id: "t-s",
			attack: 4,
			health: 5,
			reflect: 0,
