...
describe('Veteran', () => {
  it('chooses the intent maximizing evaluate()', () => {
    ...
    addCreature(game, 0, { id: 't-001', attack: 3, health: 3, keywords: ['charge'] });
    ...
    expect(intent).toMatchObject({ kind: 'attack', target: { type: 'hero', player: 1 } });
  });
  ...
  it('never produces illegal intents across 100 seeded random states', () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      for (let step = 0; step < 8; step++) {
        expect(() => game.submit(Veteran.chooseIntent(game, game.currentPlayer()))).not.toThrow();
