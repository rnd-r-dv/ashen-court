...
export const CARDS: Card[] = [
  // Commons (11)
  creature('choir-acolyte', 'Acolyte', 1, 1, 1, 2, 'common', [], [{ when: 'battlecry', effects: [heal(2)] }], 'She lights every candle in the chapel before dawn, whispering the names the choir has forgotten.'),
  spell('choir-mend', 'Mend', 1, 'common', [heal(3, 'any')], 'Where the lament is sung, torn flesh knits and spent breath returns.'),
  creature('choir-sergeant', 'Sergeant of the Pale', 2, 2, 2, 3, 'common', ['ward'], [], 'He has stood guard since the bells first tolled, and he will stand guard after the last echo dies.'),
  ...
  creature('choir-warden', 'Warden', 3, 2, 3, 4, 'common', ['taunt'], [], 'The gate of the pale cathedral has no lock. It has him.'),
  spell('choir-chant', 'Chant of Rest', 3, 'common', [heal(5)], 'Sleep is the choir\u2019s oldest hymn, and the dead sing it best.'),
  creature('choir-praetor', 'Praetor', 4, 3, 4, 5, 'common', [], [{ when: 'battlecry', effects: [silence('enemyCreature')] }], 'He keeps the choir\u2019s ledger in cold iron, and every debt is paid in kind.'),
  ...
  creature('choir-luminarch', 'Luminarch', 5, 4, 4, 6, 'common', [], [{ when: 'battlecry', effects: [draw(1)] }], 'The tallest candle burns coldest, and its shadow is the whole cathedral.'),
  // Rares (5)
  spell('choir-revelation', 'Revelation', 4, 'rare', [draw(3)], 'What the choir knows, it knows slowly \u2014 all at once, and without mercy.'),
  creature('choir-seraph', 'Seraph of Lament', 6, 5, 6, 6, 'rare', ['lifesteal'], [], 'She weeps for the wounded, and every wound she deals she carries home like a hymn.'),
  ...
  creature('choir-martyr', 'Martyr', 5, 3, 4, 5, 'rare', [], [{ when: 'deathrattle', effects: [heal(5)] }], 'She spends herself so that others may sing another verse.'),
  ...
  // Epics (3)
  creature('choir-exorcist', 'Exorcist', 6, 4, 5, 7, 'epic', [], [{ when: 'battlecry', effects: [destroy('enemyCreature')] }], 'He has cast out worse than you, and he remembers them all by name.'),
  spell('choir-truth', 'Truth Unveiled', 6, 'epic', [draw(4), heal(4)], 'The veil was never meant to hold. It was only ever meant to delay.'),
  creature('choir-lightbringer', 'Lightbringer', 7, 6, 7, 8, 'epic', ['taunt'], [], 'He bears the light not because it is his, but because it is needed.'),
  // Legendaries (2)
  creature('choir-lady', 'Lady of the Pale Choir', 9, 6, 7, 8, 'legendary', ['taunt'], [{ when: 'startOfTurn', effects: [heal(4)] }], 'The choir began as her lament, and it will end the same way \u2014 softly, and only when she wills it.'),
