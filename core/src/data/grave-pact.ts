...
export const CARDS: Card[] = [
  ...
  creature('pact-imp', 'Blood Imp', 2, 3, 1, 2, 'common', [], [], 'Small, sharp, and eager to be repaid in pain.'),
  creature('pact-leech', 'Leech', 2, 2, 2, 2, 'common', ['lifesteal'], [], 'It drinks deeply, and the wound closes on the other side of the bargain.'),
  spell('pact-bond', 'Blood Bond', 3, 'common', [buff(3, 3, 'friendlyCreature', 1), dmg(2, 'self')], 'Two hearts beat as one; the second pays for the first’s strength.'),
  creature('pact-masochist', 'Masochist', 3, 3, 3, 4, 'common', [], [{ when: 'onDamage', effects: [draw(1)] }], 'Pain is a language, and the Masochist is fluent. Every wound is a whispered word of power.'),
  creature('pact-ravager', 'Ravager', 4, 5, 3, 3, 'common', [], [{ when: 'battlecry', effects: [dmg(2, 'self'), draw(1)] }], 'It spends its own flesh as recklessly as it spends the enemy’s.'),
  ...
  creature('pact-cultist', 'Cultist', 4, 4, 3, 4, 'common', [], [{ when: 'deathrattle', effects: [draw(1)] }], 'Every scar is a promise kept. The pact remembers each one.'),
  ...
  creature('pact-fiend', 'Fiend', 5, 6, 3, 4, 'rare', [], [{ when: 'battlecry', effects: [dmg(2, 'self'), draw(2)] }], 'Bound by blood and kept by suffering, it bites the hand that feeds it — then asks for more.'),
  ...
  creature('pact-dread', 'Dreadknight', 6, 6, 5, 6, 'rare', [], [{ when: 'battlecry', effects: [dmg(2, 'self'), { kind: 'refillMana', value: 2 }] }], 'It rides at the head of the procession every pact eventually joins.'),
  // Epics (3)
  ...
  creature('pact-lord', 'Lord of the Pact', 8, 8, 7, 8, 'epic', [], [{ when: 'startOfTurn', effects: [dmg(1, 'self'), draw(1)] }], 'The Lord wrote the first clause in his own blood, and has been paying for it ever since.'),
  // Legendaries (2)
  creature('pact-morticia', 'Morticia Gravefall', 9, 7, 6, 9, 'legendary', [], [{ when: 'battlecry', effects: [dmg(3, 'self'), dmg(3, 'allEnemies')] }], 'Morticia Gravefall pays the blood toll herself — and the field answers in kind.'),
