...
import {
  ...
  BONE_HORDE_HERO, BONE_HORDE_DECK,
  GRAVE_PACT_HERO, GRAVE_PACT_DECK,
  NIGHT_COVEN_HERO, NIGHT_COVEN_DECK,
...
describe('decks 7-9 (Bone Horde, Grave Pact, Night Coven)', () => {
    ...
    describe(name, () => {
      ...
      it('hero power works in a real game', () => {
        ...
        if (name === 'night coven') {
          ...
          const victim = addCreature(game, 1, { id: 'hex-target', attack: 1, health: 1, keywords: [] });
          game.submit({ kind: 'heroPower', target: { type: 'creature', id: victim.id } });
          expect(game.state.players[1].board.find(c => c.id === victim.id)).toBeUndefined();
        }
      });
  ...
  describe('negative buffs kill creatures', () => {
    it('coven-nightmare buff-3/-3 on an enemy 1/1 kills it (board membership)', () => {
      ...
      toMain(game);
      const victim = addCreature(game, 1, { id: 'nightmare-victim', attack: 1, health: 1, keywords: [] });
    ...
    it('coven-wither buff-2/-2 on a 2/2 enemy creature kills it too', () => {
      const game = newGame(NIGHT_COVEN_HERO, NIGHT_COVEN_DECK);
      toMain(game);
      const victim = addCreature(game, 1, { id: 'wither-victim', attack: 2, health: 2, keywords: [] });
  ...
  describe('choice + own-hero mixed cards (amendment 10)', () => {
    it('pact-lifeleech damages the chosen enemy creature AND heals own hero', () => {
      ...
      toMain(game);
      const victim = addCreature(game, 1, { id: 'enemy-bleeder', attack: 2, health: 5 });
      game.state.players[0].hand.unshift('pact-lifeleech');
      game.state.players[0].mana = 10;
      game.state.players[0].hero.hp = 28;
      game.submit({ kind: 'playCard', handIndex: 0, target: { type: 'creature', id: victim.id } });
      expect(game.state.players[1].board.find(c => c.id === victim.id)!.health).toBe(2);   // dmg3(ec) hit the target
  ...
  describe('destroy removal (board membership)', () => {
    it('pact-sacrifice destroys a chosen friendly creature and draws 2', () => {
      const game = newGame(GRAVE_PACT_HERO, GRAVE_PACT_DECK);
      toMain(game);
      const minion = addCreature(game, 0, { id: 'own-imp', attack: 3, health: 2, keywords: [] });
      game.state.players[0].hand.unshift('pact-sacrifice');
      game.state.players[0].mana = 10;
      const handBefore = game.state.players[0].hand.length;
      game.submit({ kind: 'playCard', handIndex: 0, target: { type: 'creature', id: minion.id } });
      expect(game.state.players[0].board.find(c => c.id === minion.id)).toBeUndefined();  // removed
    ...
    it('coven-glare destroys the chosen enemy creature AND debuffs all enemy creatures', () => {
      const game = newGame(NIGHT_COVEN_HERO, NIGHT_COVEN_DECK);
      toMain(game);
      const doomed = addCreature(game, 1, { id: 'doomed-one', attack: 2, health: 3, keywords: [] });
      const survivor = addCreature(game, 1, { id: 'survivor-one', attack: 3, health: 5, keywords: [] });
      game.state.players[0].hand.unshift('coven-glare');
      game.state.players[0].mana = 10;
      game.submit({ kind: 'playCard', handIndex: 0, target: { type: 'creature', id: doomed.id } });
      expect(game.state.players[1].board.find(c => c.id === doomed.id)).toBeUndefined();  // destroy(ec) removed the target
      const surv = game.state.players[1].board.find(c => c.id === survivor.id)!;
