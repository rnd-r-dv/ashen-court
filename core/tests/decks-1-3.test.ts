...
import {
  ...
  EMBER_COURT_HERO, EMBER_COURT_DECK,
  HOLLOW_CHOIR_HERO, HOLLOW_CHOIR_DECK,
  VERMIN_SWARM_HERO, VERMIN_SWARM_DECK,
...
describe('decks 1-3 (Ember Court, Hollow Choir, Vermin Swarm)', () => {
  for (const { name, HERO, DECK } of decks) {
    ...
    describe(name, () => {
      ...
      it('hero power works in a real game', () => {
        ...
        if (name === 'ember court') expect(game.state.players[1].hero.hp).toBe(29);            // Ember Bolt dmg1
        if (name === 'hollow choir') expect(game.state.players[0].hero.hp).toBe(30);           // Lullaby heal2(h)
        if (name === 'vermin swarm') expect(game.state.players[0].board.length).toBe(1);       // Rat Call summon(token-rat)
      });
  ...
  describe('hero/self auto-resolve for mixed cards (controller ruling)', () => {
    ...
    it('choir-verdict destroys the chosen enemy creature AND heals own hero', () => {
      ...
      toMain(game);
      const victim = addCreature(game, 1, { id: 'enemy-cultist', attack: 3, health: 6 });
      game.state.players[0].hand.unshift('choir-verdict');
      game.state.players[0].mana = 10;
      game.state.players[0].hero.hp = 24;
      game.submit({ kind: 'playCard', handIndex: 0, target: { type: 'creature', id: victim.id } });
      expect(game.state.players[1].board.find(c => c.id === victim.id)).toBeUndefined();  // destroy(ec) removed the target
