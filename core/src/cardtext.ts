...
export const KEYWORD_TEXT: Record<Keyword, string> = {
	taunt:
		"Enemies must attack this creature before your hero or your other creatures.",
	rush: "Can attack enemy creatures the turn it is summoned.",
	charge:
		"Can attack anything, including the enemy hero, the turn it is summoned.",
	windfury: "Can attack twice each turn.",
	lifesteal:
		"Damage this creature deals also restores that much health to your hero.",
...
): string => (n === 1 ? singular : pluralForm);
...
export function cardText(card: Card): string {
	...
	return TRIGGER_ORDER.map((when) => {
		...
		return effects.length > 0
			? `${TRIGGER_LABEL[when]}: ${effects.map(effectText).join(" ")}`
			: "";
	})
		.filter(Boolean)
		.join("\n");
