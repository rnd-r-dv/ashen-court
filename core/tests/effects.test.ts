...
describe("dealDamage", () => {
	...
	it("damaging a hero to 0 ends the game", () => {
		...
		applyEffect(game, ctx, {
			...
			value: 3,
			target: "allEnemies",
		}); // allEnemies hits the enemy hero (enemy board empty)
	...
	it("hero damage emits heroDamaged and summarize counts it on a real log (audit 01 I1)", () => {
		...
		expect(
			evts.some(
				(e) => e.type === "heroDamaged" && e.player === 1 && e.amount === 2,
			),
		).toBe(true);
		expect(
			evts.some(
...
describe("heal / buff / summon / gainMana / freeze / destroy / copyCard / giveKeyword", () => {
	...
	it("buff adds attack and health, and maxHealth", () => {
		...
		expect(c.board[0]!.maxHealth).toBe(5);
	});
	...
	it("silence preserves permanent stat modifications, reflect included (Task 1)", () => {
		...
		applyEffect(
			game,
			ctx,
			{ kind: "buff", value: 1, value3: 1, target: "anyCreature" },
		...
		applyEffect(
			game,
			ctx,
			{ kind: "silence", target: "anyCreature" },
			{ type: "creature", id: c.id },
		);
	...
	it("gainMana adds an empty crystal, refillMana fills one", () => {
		const game = g();
		game.state.phase = "main";
		game.state.players[0].maxMana = 3;
		game.state.players[0].mana = 0;
		applyEffect(game, ctx, { kind: "gainMana", value: 1 });
		expect(game.state.players[0].maxMana).toBe(4);
		expect(game.state.players[0].mana).toBe(0);
	...
	it("giveKeyword shield grants a real absorb — the shields field increments and damage is absorbed", () => {
		...
		applyEffect(
			game,
			ctx,
			{ kind: "giveKeyword", keyword: "shield", target: "anyCreature" },
			{ type: "creature", id: c.id },
		);
		...
		applyEffect(
			game,
			ctx,
			{ kind: "dealDamage", value: 1, target: "anyCreature" },
			{ type: "creature", id: c.id },
		);
	...
	it("giveKeyword ward sets warded", () => {
		...
		applyEffect(
			game,
			ctx,
			{ kind: "giveKeyword", keyword: "ward", target: "anyCreature" },
			{ type: "creature", id: c.id },
		);
	...
	it("giveKeyword windfury increments attacksLeft for an immediate extra swing", () => {
		...
		applyEffect(
			game,
			ctx,
			{ kind: "giveKeyword", keyword: "windfury", target: "anyCreature" },
			{ type: "creature", id: c.id },
		);
