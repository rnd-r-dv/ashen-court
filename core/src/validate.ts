...
export const KEYWORD_COST: Record<Keyword, number> = {
  taunt: 1, rush: 1, charge: 2, windfury: 2, lifesteal: 1, ward: 1, shield: 1, venom: 2, stealth: 1,
};

const CREATURE_ONLY_KEYWORDS: Keyword[] = ['taunt', 'rush', 'charge', 'windfury', 'lifesteal', 'ward', 'shield', 'venom', 'stealth'];
...
export function validateCard(card: Card): ValidationIssue[] {
  ...
  const TARGET_KINDS: EffectKind[] = ['dealDamage', 'heal', 'buff', 'freeze', 'destroy', 'silence', 'giveKeyword', 'returnToHand', 'spellPower'];
