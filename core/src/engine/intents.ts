...
export function playEffectiveCost(
	game: Game,
	card: Card,
	me: PlayerIndex,
): number {
	...
	if (card.type === "spell") {
		...
		card.type === "creature" &&
		isMostExpensiveCreatureInHand(game, me, card)
	) {
...
export function isMostExpensiveCreatureInHand(
	game: Game,
	me: PlayerIndex,
	card: Card,
): boolean {
	for (const id of game.state.players[me].hand) {
		...
		if (other && other.type === "creature" && other.cost > card.cost)
			return false;
	}
	return true;
}
...
export function validatePlayCard(
	...
	if (
		card.type === "creature" &&
		p.board.filter((c) => !c.token).length >= BOARD_CAP
	)
	...
	return validateEffectTargets(
		game,
		me,
		[...card.effects, ...battlecryEffects(card)],
		intent.target,
	);
}
...
export function requiredConsumeTokens(card: Card): number {
	...
	return immediate.reduce(
		(n, s) => (s.kind === "consume" ? n + (s.value ?? 1) : n),
		0,
	);
}
...
function validateTarget(
	...
	if (target.type === "hero") {
		...
		return kind === "any"
			? null
			: `Illegal target: hero is not a valid ${kind} target`;
	}
	...
	switch (kind) {
		...
		case "anyCreature":
			return null;
		case "enemyCreature":
			return c.owner === ((1 - me) as PlayerIndex)
				? null
				: "Illegal target: must be an enemy creature";
		case "friendlyCreature":
			return c.owner === me
				? null
				: "Illegal target: must be a friendly creature";
		case "friendlyDragon":
			return c.owner === me && isDragon(game, c)
				? null
				: "Illegal target: must be a friendly dragon";
		default:
...
export function legalIntents(game: Game, player: PlayerIndex): Intent[] {
	...
	for (let i = 0; i < p.hand.length; i++) {
		...
		if (!immediateConsumeAffordability(game.state, player, card).payable)
			continue;
		...
		const variants = targetVariants(game, player, [
			...card.effects,
			...battlecryEffects(card),
		]);
		if (!variants) continue; // single-target effect with no legal ref → unplayable
		for (const t of variants)
			out.push({ kind: "playCard", handIndex: i, target: t });
	}
	...
	const enemyBoard = game.state.players[enemy].board.filter(
		(c) => !c.keywords.includes("stealth"),
	);
	...
	if (!p.hero.usedPower && p.mana >= p.hero.power.cost) {
		...
		if (variants)
			for (const t of variants) out.push({ kind: "heroPower", target: t });
	}
...
function targetVariants(
	...
	for (const spec of effects)
		if (isChoiceTarget(spec.target)) choices.push(spec.target!);
	if (choices.length === 0) return [undefined];
	const refs = resolveTargets(game, me, choices[0]!).filter((ref) =>
		choices.every((kind) => validateTarget(game, me, kind, ref) === null),
	);
