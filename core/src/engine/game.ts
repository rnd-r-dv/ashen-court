...
import {
	isMostExpensiveCreatureInHand,
	legalIntents as computeLegalIntents,
	playEffectiveCost,
...
export class Game implements Resolver {
	...
	constructor(setup: MatchSetup, registry: CardRegistry) {
		if (!setup.heroes || setup.heroes.length !== 2)
			throw new Error("Two heroes required");
		...
		for (const [i, deck] of setup.decks.entries()) {
			...
			if (issues.some((iss) => iss.severity === "error")) {
				throw new Error(
					`Deck ${i} invalid: ${issues.map((iss) => iss.message).join("; ")}`,
				);
			}
		}
		...
		for (const p of players)
			for (let i = 0; i < STARTING_HAND; i++) p.hand.push(p.deck.pop()!);
		...
		this.state = {
			...
			rngState: { seed: setup.seed, calls: 0 },
			log: [],
			pendingChoice: null,
			pendingChoiceQueue: [],
		};
	...
	submit(intent: Intent): GameEvent[] {
		...
		if (pending !== null) {
			...
			if (intent.kind === "discover")
				throw new Error("No Discover choice pending");
	...
	private resolveIntent(intent: Intent, me: PlayerIndex): GameEvent[] {
		if (intent.kind === "discover") {
			...
			if (
				!Number.isInteger(intent.choice) ||
				intent.choice < 0 ||
				intent.choice >= pending.cardIds.length
			) {
				throw new Error("Bad Discover choice");
			}
			this.emit({
				type: "discoverResolved",
				player: pending.player,
				cardId: pending.cardIds[intent.choice]!,
			});
			return runQueue(this);
		}
		if (intent.kind === "mulligan") {
			...
			if (
				new Set(intent.keep).size !== intent.keep.length ||
				intent.keep.length > p.hand.length
			) {
			...
			for (const idx of kept) {
				if (idx < 0 || idx >= p.hand.length) throw new Error("Bad keep index");
			}
			...
			if (this.state.mulligansDone[0] && this.state.mulligansDone[1])
				this.startMain();
			return runQueue(this);
		}
		...
		if (intent.kind === "attack") {
			...
			const attacker = this.state.players[me].board.find(
				(c) => c.id === intent.attackerId,
			);
			if (!attacker) throw new Error("Attacker not found");
			if (!canAttack(attacker, this))
				throw new Error("Cannot attack with that creature");
			...
			if (tauntPresent(enemyBoard)) {
				...
				if (
					!effectiveKeywords(d).has("taunt") ||
					d.keywords.includes("stealth")
				)
					throw new Error("Taunt creature in the way");
			}
			...
			if (target.type === "creature") {
				const d = enemyBoard.find((c) => c.id === target.id);
				if (!d) throw new Error("Defender not found");
				if (effectiveKeywords(d).has("stealth"))
					throw new Error("Cannot target a stealthed creature");
			}
		...
		if (intent.kind === "playCard") {
			...
			if (
				card.type === "creature" &&
				isMostExpensiveCreatureInHand(this, me, card)
			)
			...
			switch (card.type) {
				...
				case "spell": {
					...
					const hasSingleTarget = card.effects.some((spec) =>
						isChoiceTarget(spec.target),
					);
					...
					if (wardRef && wardRef.type === "creature") {
						...
						if (
							warded &&
							warded.warded &&
							warded.owner === ((1 - me) as PlayerIndex)
						) {
							warded.warded = false;
							this.emit({
								type: "spellFizzled",
					...
					if (!fizzled) {
						for (const spec of card.effects) {
							...
							applyEffect(
								this,
								{ player: me, cardId: card.id },
								spec,
								specTargetRef(spec, intent.target),
							);
						}
					}
					break;
				...
				case "artifact": {
					this.emit({ type: "cardPlayed", player: me, cardId: card.id });
					p.artifacts.push({
						id: this.nextArtifactId(card.id),
						cardId: card.id,
						owner: me,
					});
					break;
		...
		if (intent.kind === "heroPower") {
			...
			const err = validateEffectTargets(
				this,
				me,
				p.hero.power.effects,
				intent.target,
			);
			if (err) throw new Error(err);
		...
		switch (evt.type) {
			...
			case "discoverOffered":
				...
				if (this.state.pendingChoice === null)
					this.state.pendingChoice = evt.choice;
			...
			case "discoverResolved":
				...
				this.state.pendingChoice =
					this.state.pendingChoiceQueue.shift() ?? null;
				break;
			case "turnEnd":
	...
	private triggerGroups(cardId: string, when: Trigger): TriggerSpec[] {
		return (this.safeCard(cardId)?.triggers ?? []).filter(
			(t) => t.when === when,
		);
	}
	...
	private fireTriggers(
		...
		for (const group of this.triggerGroups(cardId, when)) {
			for (const spec of group.effects) {
				applyEffect(
					this,
					{ player, cardId, creatureId },
					spec,
					specTargetRef(spec, explicitRef),
				);
			}
		}
	}
	...
	private nextArtifactId(cardId: string): string {
		...
		for (const p of this.state.players)
			for (const a of p.artifacts) if (a.cardId === cardId) n++;
		return `${cardId}-${n + 1}`;
	}
	...
	private dealDamage(
		...
		applyEffect(
			this,
			ctx,
			{ kind: "dealDamage", value: amount, target: "anyCreature" },
			{ type: "creature", id: target.id },
		);
	}

	/** Hero-target attack damage; same source-derived controller ruling as dealDamage. */
	private dealDamageToHero(
		source: CreatureState,
		...
		applyEffect(
			this,
			ctx,
			{ kind: "dealDamage", value: amount, target: "hero" },
			{ type: "hero", player: targetPlayer },
		);
	}
