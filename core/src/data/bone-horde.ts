...
const spell = (
  id: string, name: string, cost: number, rarity: Rarity, effects: EffectSpec[],
  flavor?: string,
): Card => ({
...
export const CARDS: Card[] = [
  ...
  creature('bone-cairn', 'Cairn', 3, 0, 4, 'common', ['taunt'], [{ when: 'deathrattle', effects: [summon('token-skeleton')] }], 'A heap of the honored dead, piled high enough to block the living.'),
  spell('bone-frenzy', 'Bone Frenzy', 3, 'common', [buff(1, 1, 'allFriendlyCreatures')], 'The horde forgets fear. It remembers only the charge.'),
  creature('bone-raider', 'Raider', 3, 3, 3, 'common', [], [], 'It plunders the living for what the dead no longer need.'),
  spell('bone-rattle', 'Rattle', 4, 'common', [dmg(2, 'allEnemies')], 'The horde rattles as one, and the sound cracks the bones of the living.'),
