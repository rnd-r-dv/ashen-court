	...
	waitFor(
		...
		return new Promise((resolve, reject) => {
			const timer = setTimeout(() => {
				this.pendings.splice(
					this.pendings.findIndex((p) => p.timer === timer),
					1,
				);
...
async function joinWhenSlotFree(
	...
	for (;;) {
		...
		if (msg.type !== "error" || !/room is full/i.test(msg.message)) {
			throw new Error(
				`joinRoom rejected: ${msg.type === "error" ? msg.message : msg.type}`,
			);
		}
		if (Date.now() >= deadline)
			throw new Error("room never freed a slot within the timeout");
...
async function makeClients(
	srv: LanServer,
): Promise<{ host: TestClient; guest: TestClient }> {
	const url = urlOf(srv);
	return {
...
async function startRoom(
	srv: LanServer,
	seed: number = SEED,
): Promise<{ host: TestClient; guest: TestClient; code: string }> {
...
describe("LAN rooms", () => {
	...
	it("intents flow: host plays a card, both clients receive identical events", async () => {
		...
		try {
			...
			expect(
				play.some((e) => e.type === "cardPlayed" && e.cardId === PLAY_CARD),
			).toBe(true);
			const hostPlayP = host.waitFor(eventsEq(play));
			const guestPlayP = guest.waitFor(eventsEq(play));
			host.send({
				type: "intent",
				intent: { kind: "playCard", handIndex: PLAY_INDEX },
			});
			const hostPlay = await hostPlayP;
			const guestPlay = await guestPlayP;
			if (hostPlay.type !== "events" || guestPlay.type !== "events")
				throw new Error("play events never arrived");
	...
	it("illegal intent gets an error reply, not a crash", async () => {
		...
		try {
			...
			host.send({
				type: "intent",
				intent: { kind: "playCard", handIndex: 99 },
			});
			const err = await errP;
			if (err.type !== "error") throw new Error("error never arrived");
	...
	it("disconnect notifies the other player", async () => {
		...
		try {
			...
			if (left.type !== "playerLeft")
				throw new Error("playerLeft never arrived");
	...
	it("host receives opponentJoined carrying the guest deck/hero (Task 45)", async () => {
		...
		try {
			...
			if (opp.type !== "opponentJoined")
				throw new Error("opponentJoined never arrived");
	...
	it("rematch requires both players and starts a new game", async () => {
		...
		try {
			...
			expect(await hostP).toMatchObject({
				type: "rematchStart",
				seed: SEED + 1,
			});
			expect(await guestP).toMatchObject({
				type: "rematchStart",
				seed: SEED + 1,
			});
	...
	it("Bug 6: a reconnect notifies the still-connected peer (opponentReconnected)", async () => {
		...
		try {
			...
			const oppSilence = host
				.waitFor((m) => m.type === "opponentJoined", 500)
				.then(
					() => {
						throw new Error(
							"host must not receive opponentJoined on a peer reconnect",
						);
					},
					() => undefined,
				);
	...
	it("Bug 8: replayed intents on reconnect are flagged, live ones are not", async () => {
		...
		try {
			...
			if (liveMsg.type !== "intent")
				throw new Error("intent echo never arrived");
	...
	it("Bug 7: a disconnect drops the departing player pending rematch request", async () => {
		...
		try {
			...
			const guestSilence = guest
				.waitFor((m) => m.type === "rematchStart", 500)
				.then(
					() => {
						throw new Error(
							"a stale rematch request from the departed player started a rematch",
						);
					},
					() => undefined,
				);
	...
	it("C1: non-object message bodies get an error reply, not a crash", async () => {
		...
		try {
			...
			for (const body of ["null", "42", "true", '"hi"', "[]"]) {
				raw.ws.send(body);
				const err = await raw.waitFor(
					(m) => m.type === "error" && m.message === "Invalid message",
				);
				if (err.type !== "error") throw new Error("error never arrived");
	...
	it("Discover: only the pending owner may resolve the choice; endTurn is rejected while pending", async () => {
		...
		try {
			...
			expect(
				resolved.some(
					(e) => e.type === "discoverResolved" && e.cardId === "neutral-scroll",
				),
			).toBe(true);
			const hostResP = host.waitFor(eventsEq(resolved));
			const guestResP = guest.waitFor(eventsEq(resolved));
			const hostIntentP = host.waitFor(
				intentEq({ kind: "discover", choice: 1 }),
			);
			const guestIntentP = guest.waitFor(
				intentEq({ kind: "discover", choice: 1 }),
			);
			guest.send({ type: "intent", intent: { kind: "discover", choice: 1 } });
			const hostRes = await hostResP;
			const guestRes = await guestResP;
			if (hostRes.type !== "events" || guestRes.type !== "events")
				throw new Error("events never arrived");
			expect(guestRes.events).toEqual(hostRes.events);
			expect(await hostIntentP).toMatchObject({
				type: "intent",
				intent: { kind: "discover", choice: 1 },
			});
			expect(await guestIntentP).toMatchObject({
				type: "intent",
				intent: { kind: "discover", choice: 1 },
			});
	...
	it("Discover: playCard + discover intents enter the log and reconnect replays them to the same state", async () => {
		...
		try {
			...
			const play = mirror.submit({ kind: "playCard", handIndex });
			const offered = play.find(
				(e): e is Extract<GameEvent, { type: "discoverOffered" }> =>
					e.type === "discoverOffered",
			);
			...
			expect(
				disc.some((e) => e.type === "discoverResolved" && e.cardId === picked),
			).toBe(true);
			...
			const guestPlay = mirror.submit({
				kind: "playCard",
				handIndex: guestPlayIndex,
				target: { type: "hero", player: 0 },
			});
...
describe("LAN rooms — address-carrying join codes", () => {
	...
	it("still reports Room not found for a well-formed code naming no room", async () => {
		...
		try {
			...
			guest.send({
				type: "joinRoom",
				code: formatJoinCode("ZZZZ", "10.42.0.116"),
				deckIds: DECK,
				customCards: [],
				heroId: HERO_NAME,
			});
