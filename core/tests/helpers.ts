...
export const hero = (name: string): HeroSpec => ({
	name,
	power: {
		name: "Test Power",
		cost: 2,
		effects: [{ kind: "dealDamage", value: 1, target: "any" }],
	},
});
...
export function addCreature(
	...
	game.registry.register(
		partial.trigger
			? {
					...base,
					triggers: [{ when: partial.trigger, effects: partial.effects ?? [] }],
				}
			: base,
	);
	return creature;
