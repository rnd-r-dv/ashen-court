...
export function isChoiceTarget(target: EffectTarget | undefined): boolean {
	return (
		target !== undefined &&
		SINGLE_TARGET_TARGETS.has(target) &&
		target !== "hero" &&
		target !== "self"
	);
}
	...
	switch (spec.kind) {
		case "dealDamage": {
			...
			if (amount > 0) {
				...
				if (dealt > 0 && hasLifesteal(game, ctx))
					healHero(game, ctx.player, dealt);
			}
			break;
		}
		case "draw": {
		...
		case "discountMostExpensive":
			game.state.players[ctx.player].hero.discountMostExpensive +=
				spec.value ?? 0;
			break;
		...
		case "discover": {
			...
			const eligible = [...registryOf(game).pool().values()].filter(
				(card) => card.archetype !== "token" && card.id !== "mana-surge",
			);
			if (eligible.length < 3)
				throw new Error("Not enough eligible cards to Discover");
			const bag = [...eligible];
			...
			push(game, {
				type: "discoverOffered",
				choice: { kind: "discover", player: ctx.player, cardIds },
			});
			break;
		}
	}
...
export function resolveTargets(
	...
	switch (target) {
		...
		case "allEnemies":
			return [
				{ type: "hero" as const, player: enemy },
				...creatureRefs(hostile),
			];
...
function registryOf(game: Resolver): CardRegistry {
	...
	if (!reg)
		throw new Error(
			"applyEffect requires a resolver exposing a card registry (Game does)",
		);
	return reg;
}
...
function hasLifesteal(game: Resolver, ctx: EffectCtx): boolean {
	...
	return (
		findCreature(game, ctx.creatureId)?.keywords.includes("lifesteal") ?? false
	);
}
...
function healHero(game: Resolver, player: PlayerIndex, amount: number): void {
	...
	hero.hp += healed;
	if (healed > 0)
		push(game, { type: "heroHealed", player, amount: healed, hp: hero.hp });
}
...
function randomEnemyCreatureCardId(
	game: Resolver,
	player: PlayerIndex,
): string | undefined {
	const enemy = (1 - player) as PlayerIndex;
