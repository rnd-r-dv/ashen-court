...
describe("effectText — one template per EffectKind (Task 43)", () => {
	it("dealDamage", () => {
		expect(effectText({ kind: "dealDamage", value: 5, target: "any" })).toBe(
			"Deal 5 damage to any target.",
		);
	});
	it("heal", () => {
		expect(effectText({ kind: "heal", value: 3, target: "hero" })).toBe(
	...
	it("buff — value2 defaults to value (Attack + Health named)", () => {
		expect(
			effectText({ kind: "buff", value: 2, target: "friendlyCreature" }),
		).toBe("Give a friendly creature +2 Attack and +2 Health.");
	});
	it("buff — negatives render as U+2212", () => {
		...
		).toBe("Give an enemy creature \u22121 Attack and \u22121 Health.");
	});
	...
	it("buff — non-zero deltas in Attack, Reflect, Health order", () => {
		...
		).toBe("Give a friendly creature +2 Attack and +1 Reflect.");
		expect(
			effectText({
				kind: "buff",
				value3: 1,
		...
		).toBe("Give all friendly creatures +1 Reflect and +2 Health.");
		expect(
			effectText({ kind: "buff", value2: 3, target: "friendlyCreature" }),
		).toBe("Give a friendly creature +3 Health.");
	});
	...
	it("summon — single token, display name from token map", () => {
		expect(effectText({ kind: "summon", cardId: "token-skeleton" })).toBe(
			"Summon 1 Skeleton.",
		);
	});
	it("summon — plural count", () => {
		expect(effectText({ kind: "summon", cardId: "token-rat", value: 3 })).toBe(
			"Summon 3 Giant Rats.",
		);
	});
	it("summon — unknown token id falls back to the raw cardId", () => {
		expect(effectText({ kind: "summon", cardId: "mystery-token" })).toBe(
			"Summon 1 mystery-token.",
		);
	});
	it("gainMana — singular", () => {
		expect(effectText({ kind: "gainMana", value: 1 })).toBe(
			"Gain 1 empty mana crystal.",
		);
	});
	it("gainMana — plural", () => {
		expect(effectText({ kind: "gainMana", value: 2 })).toBe(
	...
	it("freeze", () => {
		expect(effectText({ kind: "freeze", target: "anyCreature" })).toBe(
			"Freeze a creature.",
		);
	});
	it("destroy", () => {
		expect(effectText({ kind: "destroy", target: "enemyCreature" })).toBe(
			"Destroy an enemy creature.",
		);
	});
	it("copyCard — with cardId", () => {
		expect(effectText({ kind: "copyCard", cardId: "token-wisp" })).toBe(
			"Add a copy of Choir Spirit to your hand.",
		);
	});
	it("copyCard — no cardId (random enemy creature)", () => {
		expect(effectText({ kind: "copyCard" })).toBe(
	...
	it("giveKeyword", () => {
		...
		).toBe("Give a friendly creature Shield.");
	});
	it("discountMostExpensive", () => {
		expect(effectText({ kind: "discountMostExpensive", value: 1 })).toBe(
			"Your most expensive creature costs 1 less this turn.",
		);
	});
	it("discountNextSpell", () => {
		expect(effectText({ kind: "discountNextSpell", value: 2 })).toBe(
			"Your next spell costs 2 less this turn.",
		);
	});
	it("discover (Task 1)", () => {
...
describe("cardText — target-name mapping through multi-effect cards (Task 43)", () => {
				...
				spellCard([
					{
						kind: "dealDamage",
						value: 1,
						target: target as EffectSpec["target"],
					},
				]),
			);
	...
	it("creature triggers group by name, joined with newlines", () => {
		const c = creatureCard([
			...
			{
				when: "battlecry",
				effects: [{ kind: "dealDamage", value: 3, target: "allEnemies" }],
			},
		]);
	...
	it("startOfTurn / endOfTurn / onDamage labels", () => {
		const c = creatureCard([
			...
			{
				when: "onDamage",
				effects: [{ kind: "heal", value: 1, target: "hero" }],
			},
		]);
		expect(cardText(c)).toBe(
	...
	it("multiple effects inside one trigger group join with a space", () => {
		...
		expect(cardText(c)).toBe(
			"Battlecry: Deal 2 damage to any target. Freeze a creature.",
		);
	});
});
...
describe("cardText — vanilla and keyword-only cards (Task 43)", () => {
	...
	it("artifact with no triggers returns empty string", () => {
		expect(
			cardText(card({ type: "artifact", triggers: [], effects: [] })),
		).toBe("");
	});
});
...
describe("cardText — real curated cards (Task 43)", () => {
	it("ember-cauterize exact string", () => {
		...
		expect(cardText(pool.get("ember-cauterize")!)).toBe(
			"Deal 6 damage to any target. Overload: 1.",
		);
	});
	...
	it("token display names resolve for every summon cardId in the pool", () => {
		...
		for (const c of buildPool()) {
			for (const e of c.effects) {
				if (e.kind === "summon" && e.cardId)
					expect(tokenIds.has(e.cardId), `${c.id}: ${e.cardId}`).toBe(true);
			}
		}
	});
});
...
describe("keyword text", () => {
	it("distinguishes ward from shield", () => {
		expect(KEYWORD_TEXT.ward).toBe(
			"Absorbs the next enemy spell or effect that targets this creature.",
		);
		expect(KEYWORD_TEXT.shield).toBe(
			"Absorbs the next instance of damage from any source.",
		);
	});
...
describe("heroPowerText (Task 43)", () => {
	it("renders the power effect list like a spell", () => {
		...
		expect(heroPowerText(power)).toBe(
