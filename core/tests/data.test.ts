...
describe("pool", () => {
	...
	const buffPairsOf = (c: Card): Array<[number, number, number | undefined]> =>
		[...(c.effects ?? []), ...(c.triggers ?? []).flatMap((t) => t.effects)]
			...
			.map((e) => [e.value ?? 0, e.value2 ?? e.value ?? 0, e.value3]);
	const APPROVED_BUFF_VALUE3: Record<
		string,
		...
		it("every curated buff and both buffing hero powers carry the exact approved value3", () => {
			...
			for (const [i, hero] of HEROES.entries()) {
				const pairs = hero.power.effects
					...
					.map(
						(e) =>
							[e.value ?? 0, e.value2 ?? e.value ?? 0, e.value3] as [
								number,
								number,
								number | undefined,
							],
				...
				if (pairs.length) {
					const key =
						hero.name === "Seraphina Skywing"
							? "hero:dragon"
							: hero.name === "Morwenna Hex"
								? "hero:coven"
								: `hero:${i}`;
					actual[key] = pairs;
				}
	...
	it("discover cards keep their immutable ids, contain Discover, and stay deck-legal (Task 1)", () => {
		...
		expect(meditate.effects).toEqual([
			{ kind: "discover" },
			{ kind: "gainMana", value: 1 },
		]);
		expect(candle.effects).toEqual([
			{ kind: "discover" },
			{ kind: "heal", value: 2, target: "hero" },
		]);
		...
		for (const def of decks) {
			...
			expect(
				validateDeck(expandDeck(def), reg.pool()).filter(
					(i) => i.severity === "error",
				),
			).toEqual([]);
		}
